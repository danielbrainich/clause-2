...
export async function GET(req) {
  ...
  refreshUrl.searchParams.set("days", "2");
  refreshUrl.searchParams.set("limit", "200");
  refreshUrl.searchParams.set("pages", "4");
  refreshUrl.searchParams.set("confirm", "0");  // fast
  try {
    const r = await fetch(refreshUrl.toString(), { cache: "no-store" });
    const upstream = await r.json().catch(() => ({}));
    return NextResponse.json(
      { ok: r.ok, ran: "ethics-refresh-daily", params: Object.fromEntries(refreshUrl.searchParams), upstream },
